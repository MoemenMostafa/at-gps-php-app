	...
	$yesterday = $_GET['date'];
        ...
        if ($resultMain && $speedZonesResult) {
			...
			while ($speedZones = mysql_fetch_assoc($speedZonesResult)){
				...
				$points = explode("),(",$speedZones['points']);
				...
				// 	print_r($polygon);
				// 	echo"<br>";
				// 	$point = "30.0810420 30.9938650";
				// 	echo $geoFence = $pointLocation->pointInPolygon($point, $polygon);
				// } 
			}
...
#print_r($speedZone);
			...
			//while ($row = mysql_fetch_assoc($resultMain)){
					...
					$vehicle_id = 299;
					echo $vehicle_id."\r\n";
                    $query = "Select id, latitude, longitude, gps_datetime, speed FROM vehicle_points_".$vehicle_id." WHERE gps_datetime LIKE '$yesterday%' AND gps_datetime NOT LIKE '20______000000'";
				...
				if ($result) {
                                        	...
                                        	if ($row2['speed'] > $minSpeedLimit){
												...
												foreach($speedZone as $zone){
													...
													if($row2['speed']>$zone['speed_limit'] && $geoFence == "inside"){
														$violation = true;
														if ($zone['id']==9) echo "<br>".$row2['id'].":",$row2['latitude'].",".$row2['longitude']."</br>";
														...
														if ($prevLat[$zone['id']] && $prevLong[$zone['id']]){
															if (distance($prevLat[$zone['id']],$prevLong[$zone['id']],$row2['latitude'],$row2['longitude'])<=0.5){
                                        ...
                                        foreach($speedZone as $zone){
											echo "Violation distance in {$zone['id']} Zone= ".$distance[$zone['id']]."<br>\r\n";
                                        	echo "Violation maxSpeed in {$zone['id']} Zone = ".$maxSpeed[$zone['id']]."<br>\r\n";
