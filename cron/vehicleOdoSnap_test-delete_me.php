...
for ($i = -62; $i <= 0; $i++) {
	...
	$yesterday = date("Ymd", strtotime("-1 days", strtotime ($today) ));
	...
	$yesterday = $_GET['date'];
        ...
        if ($resultMain) {
			...
			while ($row = mysql_fetch_assoc($resultMain)){
					$vehicle_id = $row['id'];
				// if ($vehicle_id == 312) {
                    $query = "Select latitude, longitude, gps_datetime FROM vehicle_points_".$vehicle_id." WHERE gps_datetime BETWEEN '".$yesterday."220000' and '".$today."220000' AND gps_datetime NOT LIKE '%000000'";
					...
					if ($result) {
                                        ...
                                        if (!$action) {
											...
											echo $vehicle_id . '|' . $distance[$vehicle_id] . '|' .  $gpsDateTime . "\r\n";
