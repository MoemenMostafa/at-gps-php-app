...
if ($dataProvider->totalItemCount > 0 && $_POST['Vehicle']['id']){
	...
	for($i=0;$i<count($data);$i++){
			...
			#echo $data[$i]->attributes['id'].":".$totalDistance."<br/>\r\n";
			/*-- Get all the points with that is after user specified distance and 0 speed --*/
			if($tripDistanceChecker > $userStopDistance  && $data[$i]->attributes['speed'] == 0){
				#echo $i."\r\n";
				$stopDistance = NULL;
				...
				/*------------------------------------------------------------------*/
				while (strtotime($data[$l]->attributes['gps_datetime']) < strtotime($data[$i]->attributes['gps_datetime']) + $userStopDuration){
					#echo "stopdatetime: ".strtotime($data[$l]->attributes['gps_datetime'])."\r\n";
					#echo "datetime: ".strtotime($data[$i]->attributes['gps_datetime'])."\r\n";
	...
	$dataH=array('no'=>$vehicle['serial'],'tripDuration'=>$tripDuration,'type'=>$vehicle['name'],'actualTripDuration'=>"",'layer1'=>$legalMaxSpeed1,'layer2'=>$legalMaxSpeed2,'layer3'=>$legalMaxSpeed3,'totalDistance'=>round(array_sum($stopDistances)));
