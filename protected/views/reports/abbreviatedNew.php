...
if ($dataProvider->totalItemCount > 1 && $_POST['Vehicle']['id']){
	...
	// MEMORY OPTIMIZATION: Raw SQL instead of CActiveRecord objects.
	// Each CActiveRecord carries the full Yii ORM (~5-10KB per object).
	// For 15,000 rows that's 150MB+ in ORM overhead alone.
	// Raw SQL queryAll() returns plain arrays - ~20x less memory.
	// We wrap each row in a tiny stdClass with ->attributes so
	// ALL existing $data[$i]->attributes['field'] code works unchanged.
	set_time_limit(0);
	ini_set('memory_limit', '512M');

	$vehicleId = (int)$_POST['Vehicle']['id'];
	$table = 'vehicle_points_' . $vehicleId;
	$from2 = explode("/", $_POST['Vehicle']['dateFrom']);
	$from2 = trim($from2[2]) . trim($from2[1]) . trim($from2[0]) . "000000";
	$dateFrom2 = new DateTime($from2, new DateTimeZone('Africa/Cairo'));
	$dateFrom2->setTimezone(new DateTimeZone('UTC'));
	$fromUTC = $dateFrom2->format('YmdHis');
	$to2 = explode("/", $_POST['Vehicle']['dateTo']);
	$to2 = trim($to2[2]) . trim($to2[1]) . (trim($to2[0])) . "235959";
	$dateTo2 = new DateTime($to2, new DateTimeZone('Africa/Cairo'));
	$dateTo2->setTimezone(new DateTimeZone('UTC'));
	$toUTC = $dateTo2->format('YmdHis');

	$sql = "SELECT t.* FROM `{$table}` t
	        WHERE t.gps_datetime BETWEEN :from AND :to
	          AND t.gps_datetime NOT LIKE '20______000000'
	        ORDER BY t.gps_datetime ASC";

	$rawRows = Yii::app()->db->createCommand($sql)->queryAll(true, array(
		':from' => $fromUTC,
		':to'   => $toUTC,
	));
	...
	// === PASS 1: Populate ALL player/map data - no skipping, every point ===
	// This guarantees the JS player has gap-free sequential data.
	for ($i = 0; $i < $countData - 1; $i++) {
		$js_date = reformDateTime($data[$i]->attributes['gps_datetime'], Yii::app()->user->timezone);
		...
		$distance[$i]   = round(distance(
			$data[$i]->attributes['latitude'],   $data[$i]->attributes['longitude'],
			$data[$i+1]->attributes['latitude'], $data[$i+1]->attributes['longitude']
		) * 1000);
		$stopSpeeds[$i] = $data[$i]->attributes['speed'];
		$coords[]       = new EGMapCoord($data[$i]->attributes['latitude'], $data[$i]->attributes['longitude']);
	}
	// Add the very last point to jsData (no distance calculation needed for last point)
	$last = $countData - 1;
	$js_date = reformDateTime($data[$last]->attributes['gps_datetime'], Yii::app()->user->timezone);
	$jsData[$last]['latitude']    = $data[$last]->attributes['latitude'];
	$jsData[$last]['longitude']   = $data[$last]->attributes['longitude'];
	$jsData[$last]['speed']       = $data[$last]->attributes['speed'];
	$jsData[$last]['direction']   = $data[$last]->attributes['direction'];
	$jsData[$last]['gps_datetime']= $js_date;
	$stopSpeeds[$last] = $data[$last]->attributes['speed'];
	$coords[] = new EGMapCoord($data[$last]->attributes['latitude'], $data[$last]->attributes['longitude']);
	$distance[$last] = 0;

	// === PASS 2: Stop detection only (may use $i=$l jumps freely) ===
	for ($i = 0; $i < $countData; $i++) {
		$ignitionStat = ignitionStatBol($data[$i]->attributes['input_status']);

		if ($ignitionStat == 0 || $data[$i]->attributes['speed'] < 3) {
			$stopDistance = 0;
			...
			/* that point for the specified time and the specified distance)      */
			/*-------------------------------------------------------------------*/
			while ($l < $countData - 1 && $stopDistance < $userStopDistance) {
				$ignitionAtL = ignitionStatBol($data[$l]->attributes['input_status']);
				// Vehicle is moving again — break, it's not a stop
				if ($ignitionAtL == 1 && $data[$l]->attributes['speed'] >= 3) {
					break;
				}
				$stopDistance += round(distance(
					$data[$l]->attributes['latitude'],   $data[$l]->attributes['longitude'],
					$data[$l+1]->attributes['latitude'], $data[$l+1]->attributes['longitude']
				) * 1000);
				$l++;
			}
			// Register as a real stop ONLY if:
			// 1. We actually advanced past at least one point ($l > $i)
			// 2. The vehicle stayed within the stop radius
			// 3. The stopped duration >= user-defined minimum
			if ($l > $i && $stopDistance < $userStopDistance &&
				strtotime($data[$l]->attributes['gps_datetime']) >= strtotime($data[$i]->attributes['gps_datetime']) + $userStopDuration) {
	...
	$mapStopPoints  = array(); // Only the stops that pass the distance filter (match the table)
	$mapStartPoint  = null;   // The first valid start point
	...
	for ($i=0;$i<count($startPoint);$i++){
		...
		// Skip rows where the travel distance is below the minimum stop distance threshold
		// (these are GPS drift artifacts, not real trips)
		$minDistKm = $userStopDistance / 1000;
		if ($stopDistances[$i] < $minDistKm) {
			continue;
		}

		// Track valid stop/start for map markers
		$mapStopPoints[] = $stopPoint[$i];
		if ($mapStartPoint === null) {
			$mapStartPoint = $startPoint[$i];
		}
	...
	$x=0;
	if(empty($mapStopPoints)){goto noData ;}
	foreach($mapStopPoints as $point){
	...
	// Create Map 1 Start marker
		...
		$startIdx = ($mapStartPoint !== null) ? $mapStartPoint : 0;
		$marker = new EGMapMarker($data[$startIdx]->attributes['latitude'], $data[$startIdx]->attributes['longitude'], array('title' => "start",'icon' => "'images/icons/markerIcons/largeTDGreenIcons/dd-start.png'"));
	...
	if($stopDuration){
		...
		$actualDuration = secToHMS($tripDuration);
	}
	...
	$dataH=array('date'=>$_POST['Vehicle']['dateFrom'].' - '.$_POST['Vehicle']['dateTo'],'no'=>$vehicle['serial'],'tripDuration'=>$tripDuration,'type'=>$vehicle['name'],'actualTripDuration'=>$actualDuration,'maxSpeed'=>$legalMaxSpeed,'totalDistance'=>round(array_sum($stopDistances)));
...
function addMarkerPlayer(map, data) {
	//create the markers
	  ...
	  var playerMarker = new MarkerWithLabel({
		map: map,
		position: point,
	   ...
	   infowindow = new google.maps.InfoWindow({
		...
		//	addressWindow(event.latLng, map);
		//});
...
function placeMarker(position, map) {
	if (markerAddress){
		markerAddress.setPosition(position);
                getAddresss(position);
		//infowindow.setContent(getAddresss(position));
	}else{
	  markerAddress = new google.maps.Marker({
        ...
        address.setAttribute("id", "addresss");
        address.setAttribute("style", "width:250px;height:50px");
        content.appendChild(title);
...
function updateMarkersPlayer(lat,long,direction,datetime,speed) {
		...
		//if (traceMarkerId){setMarkerTrace(traceMarkerId);}
  		...
  		updateVehicleInfo(markers);
  
}
function updateVehicleInfo(data){
		document.getElementById("iDateTime").innerHTML = data['datetime'];
		document.getElementById("iSpeed").innerHTML = data['speed'];
}
...
function getIcon(directionMod){
	...
	<div class="mapInfo">
		<table>
			<tr><td>Date & Time: </td><td><span id="iDateTime"></span>		</td></tr>
			<tr><td>Speed: </td><td><span id="iSpeed"></span> Km/h</td></tr>
		</table>
	</div>
